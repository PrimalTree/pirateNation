...
export function TopBar() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-ecu-purple/40 bg-black/70 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav className="flex items-center gap-3 text-sm">
          ...
          <Link href="/auth/signin" className={["text-zinc-300 hover:text-ecu-gold", !isAuthed ? "" : "hidden"].join(" ")}>Sign in</Link>
        </nav>
      </div>
    </header>
  );
